...
const suggestedTasks = Object.entries(template.suggested).map(
  ([key, task]) => ({
    ...task,
    category: "SUGGESTED",
    canSkip: true,
...
const retrospectiveTasks = Object.entries(template.retrospective).map(
  ([key, task]) => ({
    ...task,
...
- `[CUSTOM]-*` - Domain-specific prefixes (e.g., VOICE-_, DATA-_, SYNC-\*)
...
#### Decision Tracking (Use for: Architectural choices, pattern selection, approach changes)

- `decision_ids` - Array of Decision Index IDs this task implements or depends on (e.g., `["D-006", "D-007"]`)
- `alternatives_considered` - Array of rejected approaches with reasons (prevents re-litigating)
...
alternatives_considered = [
  "Destination charges — rejected: seller not merchant of record, complicates tax",
  "Separate charges + transfers — rejected: two-step process, higher Stripe fees",
]
```
